...
# Varijanta: CENE – content-enhanced (struktura + „sadržaj“ = strukturni atributi)
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
CENE (Content-enhanced Network Embedding): struktura (PPMI) + sadržaj (atributi = stepen, redovi A);
kombinovana sličnost M = PPMI + λ·(C C^T), SVD → embedding.
"""
...
CENE_LAM = 0.3 # 0.3 lambda for CENE
CENE_T = 6     # 6 steps for CENE
...
def compute_cene_embeddings(M, k=EMBED_DIM):
    ...
    M_struct = np.maximum(ppmi, 0.0)
    deg = A.sum(axis=1, keepdims=True).astype(float)
    deg = deg / (deg.max() + 1e-12)
    C = np.hstack([deg, A])
    C = C / (np.linalg.norm(C, axis=1, keepdims=True) + 1e-12)
    M_content = C @ C.T
    M_comb = M_struct + CENE_LAM * M_content
    U, S_svd, _ = np.linalg.svd(M_comb, full_matrices=False)
...
def main():
    ...
    emb = compute_cene_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (CENE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
...
Struktura: 
PPMI od S = I + P + … + P^CENE_T (random-walk, CENE_T=6) → M_struct (39×39).

Sadržaj (atributi): 
C = [normalizovan stepen | A] (39×(1+39)), redovi L2-normalizovani; 
sličnost sadržaja = M_content = C @ C.T (39×39).

Kombinovana sličnost: 
M_comb = M_struct + CENE_LAM * M_content (CENE_LAM=0.3) (39×39).

Embedding: 
SVD od M_comb, prvih k kolona U√Σ, zatim normalizacija u [0, π] (39×k).
"""
